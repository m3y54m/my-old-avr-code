...
#define US_TRIG_POS	PORTD.7
#define US_ECHO_POS	PIND.6

// Declare your global variables here
unsigned int timer1_overflowCount=0;
unsigned long echoPulseWidth=0;
...
interrupt [TIM1_CAPT] void timer1_capt_isr(void)
{
    ...
    {   
        ...
        echoPulseWidth = timer1_overflowCount*65536 + (unsigned long)ICR1L|((unsigned long)ICR1H<<8);
...
while (1)
      { 
            ...
            {              
                ...
                {    
                    lcd_clear();
                    sprintf(str,"%.6f ms",(float)echoPulseWidth*0.000125);
                    lcd_puts(str);
                    sprintf(str,"%.1f cm",((float)echoPulseWidth*0.125)/58);
                    lcd_gotoxy(0,1);
                    lcd_puts(str);
                    i=0;
                }   
