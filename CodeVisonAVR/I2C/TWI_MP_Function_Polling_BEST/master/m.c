...
unsigned char twi_exchange(unsigned char twi_slave_addr)
{
i2cStart();
i2cSend((twi_slave_addr<<1) | TWI_WRITE);
i2cSend(twi_tx_buffer[0]);
i2cStart(); /* restart, just send start again */
/* Setup and send address, with read bit */
i2cSend((twi_slave_addr<<1) | TWI_READ);
/* Now receive two bytes of temperature */
twi_rx_buffer[0] = i2cReadNoAck();
i2cStop();
return 1;
}
