...
void lcd_cmd(uint8_t cmd)	//Sends Command to LCD
{
	//4 bit part
...
//lcd_putsf(welcomeln1, 3, 1);	
void lcd_putsf(const char *flash_str, uint8_t x, uint8_t y)
{
	uint8_t i;
	lcd_gotoxy(x, y);
	for (i = 0; (uint8_t)pgm_read_byte(&flash_str[i]); i++)
	{
		lcd_putchar((uint8_t)pgm_read_byte(&flash_str[i]));
	}
}

void lcd_puts(const char *str, uint8_t x, uint8_t y)
{
	lcd_gotoxy(x, y);
	while (*str)
		lcd_putchar(*str++);
